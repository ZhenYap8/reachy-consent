...
import consent_assistant.config as config_mod


def _reload_core_tools() -> ModuleType:
    """Reload core_tools after config object has been patched."""
    for module_name in list(sys.modules):
        if module_name.startswith("consent_assistant.tools."):
            sys.modules.pop(module_name, None)
    # External file-loaded modules are registered by bare tool name.
    sys.modules.pop("ext_ping", None)

    sys.modules.pop("consent_assistant.tools.core_tools", None)
    core_tools_mod = importlib.import_module("consent_assistant.tools.core_tools")
